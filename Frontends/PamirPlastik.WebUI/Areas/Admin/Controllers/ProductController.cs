...
namespace PamirPlastik.WebUI.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Create(CreateProductDto createProductDto, IFormFile? imageFile)
        {
            ...
            {
                createProductDto.Slug_TR = createProductDto.Name_TR.ToLower().Replace(" ", "-").Replace("ı", "i").Replace("ğ", "g").Replace("ü", "u").Replace("ş", "s").Replace("ö", "o").Replace("ç", "c");
            }
            ...
            if (responseMessage.IsSuccessStatusCode)
            {
                TempData["SuccessMessage"] = "Ürün başarıyla kaydedildi.";
                return RedirectToAction("Index", "Product", new { area = "Admin" });
            }

            TempData["ErrorMessage"] = "Ürün kayıt edilemedi. Lütfen zorunlu alanları kontrol edin.";
            await GetCategoryListForDropdown();
        ...
        public async Task<IActionResult> Edit(UpdateProductDto updateProductDto, IFormFile? imageFile, string? MainImageUrl)
        {
            ...
            {
                updateProductDto.Slug_TR = updateProductDto.Name_TR.ToLower().Replace(" ", "-").Replace("ı", "i").Replace("ğ", "g").Replace("ü", "u").Replace("ş", "s").Replace("ö", "o").Replace("ç", "c");
            }
